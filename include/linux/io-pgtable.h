...
struct io_pgtable_ops {
	...
	size_t (*unmap)(struct io_pgtable *iop, unsigned long iova,
...
static inline int
iopt_map(struct io_pgtable *iop, unsigned long iova, phys_addr_t paddr,
	       size_t size, int prot, gfp_t gfp)
{
	if (!iop->ops || !iop->ops->map)
		return -EINVAL;
	return iop->ops->map(iop, iova, paddr, size, prot, gfp);
}
...
iopt_unmap(struct io_pgtable *iop, unsigned long iova, size_t size,
	   struct iommu_iotlb_gather *gather)
{
	if (!iop->ops || !iop->ops->unmap)
		return 0;
	return iop->ops->unmap(iop, iova, size, gather);
}
