...
"(/dev/smmuteX) or VFIO devices (/sys/...)                               \n\n"
...
int run_child(struct program_options *opts, struct smmute_dev *devs, int seed)
{
	...
	for (i = 0; i < nr_devs; i++) {
		...
		ret = smmute_bind(&devs[i], &new_pasid);
		if (ret) {
			pr_err("Cannot bind: %s\n", strerror(ret));
			break;
		}

		devices_bound++;

		/* Only for VFIO. With the kernel driver pasid == 0 */
