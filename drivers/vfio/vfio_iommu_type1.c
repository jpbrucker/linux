...
static void vfio_iommu_unbind_group(struct vfio_iommu_group *group, int pasid)
{
	struct vfio_sva_handle *sva;
...
static void vfio_iommu_unbind_group_all(struct vfio_iommu_group *group)
{
				  ...
				  struct vfio_iommu_group *group)
{
	...
	struct vfio_domain *domain;
	struct vfio_sva_handle *sva;
	struct vfio_iommu_group *old_group;
				       ...
				       struct vfio_domain *new_domain)
{
	...
	iommu_detach_group(cur_domain->domain, group->iommu_group);
	if (iommu_attach_group(new_domain->domain, group->iommu_group))
		goto out_reattach;
...
out_detach:
	iommu_detach_group(new_domain->domain, group->iommu_group);

out_reattach:
	if (iommu_attach_group(cur_domain->domain, group->iommu_group))
		return -EINVAL;
