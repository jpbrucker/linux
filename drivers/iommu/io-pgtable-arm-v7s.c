...
{
	return arm_v7s_map_pages(iop, iova, paddr, size, 1, prot, gfp, NULL);
}
...
static size_t arm_v7s_unmap(struct io_pgtable *iop, unsigned long iova,
			    size_t size, struct iommu_iotlb_gather *gather)
{
	return arm_v7s_unmap_pages(iop, iova, size, 1, gather);
}
...
static int __init arm_v7s_do_selftests(void)
{
	...
	for_each_set_bit(i, &cfg.pgsize_bitmap, BITS_PER_LONG) {
		size = 1UL << i;
		if (iopt_map(&iop, iova, iova, size, IOMMU_READ |
						     IOMMU_WRITE |
						     IOMMU_NOEXEC |
		...
		if (!iopt_map(&iop, iova, iova + size, size,
	...
	while (i < loopnr) {
		iova_start = i * SZ_16M;
		if (iopt_unmap(&iop, iova_start + size, size, NULL) != size)
			return __FAIL(ops);

		/* Remap of partial unmap */
		if (iopt_map(&iop, iova_start + size, size, size, IOMMU_READ, GFP_KERNEL))
			return __FAIL(ops);
	...
	for_each_set_bit(i, &cfg.pgsize_bitmap, BITS_PER_LONG) {
		...
		if (iopt_unmap(&iop, iova, size, NULL) != size)
			return __FAIL(ops);
		...
		/* Remap full block */
		if (iopt_map(&iop, iova, iova, size, IOMMU_WRITE, GFP_KERNEL))
			return __FAIL(ops);
